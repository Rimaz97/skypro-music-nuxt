    ...
    if (filtersStore.selectedFilters.genre.length > 0) {
      result = result.filter(track => {
        if (Array.isArray(track.genre)) {
          return track.genre.some(g => 
            g && filtersStore.selectedFilters.genre.includes(g.toLowerCase().trim())
          );
        } else if (track.genre) {
          return filtersStore.selectedFilters.genre.includes(track.genre.toLowerCase().trim());
        }
        return false;
      });
    }
